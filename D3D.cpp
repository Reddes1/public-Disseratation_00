...
bool D3D12Application::InitMainWindow()
{
    ...
    RECT R = { 0, 0, m_WinData.m_Width, m_WinData.m_Height };
    ...
    m_WinData.m_Window = CreateWindow("MainWnd", m_WinData.m_WindowCaption.c_str(),
        m_WinData.m_WinStyle, offsetX, offsetY, width, height, 0, 0, m_WinData.m_AppInst, 0);
    if (!m_WinData.m_Window)
    {
        MessageBox(0, "CreateWindow Failed.", 0, 0);
        return false;
    }

    ShowWindow(m_WinData.m_Window, SW_SHOW);
    UpdateWindow(m_WinData.m_Window);
...
void D3D12Application::CalculateFrameStats()
{
    ...
    if ((m_Timer.TotalTime() - timeElapsed) >= 1.0f)
    {
        ...
        std::string windowText = m_WinData.m_WindowCaption +
            "    fps: " + fpsStr +
            "   mspf: " + mspfStr;

        SetWindowTextA(m_WinData.m_Window, windowText.c_str());
...
void D3D12Application::CreateSwapChain()
{
    ...
    sd.BufferDesc.Width = m_WinData.m_Width;
    sd.BufferDesc.Height = m_WinData.m_Height;
    ...
    sd.OutputWindow = m_WinData.m_Window;
    sd.Windowed = true;
...
void D3D12Application::OnResize()
{
    ...
    ThrowIfFailed(m_SwapChain->ResizeBuffers(
        ...
        m_WinData.m_Width, m_WinData.m_Height,
        m_BackBufferFormat,
    ...
    depthStencilDesc.Width = m_WinData.m_Width;
    depthStencilDesc.Height = m_WinData.m_Height;
    ...
    m_ScreenViewport.Width = static_cast<float>(m_WinData.m_Width);
    m_ScreenViewport.Height = static_cast<float>(m_WinData.m_Height);
    m_ScreenViewport.MinDepth = 0.0f;
    m_ScreenViewport.MaxDepth = 1.0f;

    m_ScissorRect = { 0, 0, m_WinData.m_Width, m_WinData.m_Height };
}
...
void D3D12Application::SnapToRes16_9()
{
 ...
 /*       {1024, 576},*/
    ...
    int& winX = m_WinData.m_Width;

    //Solve for edge cases
    if (winX <= res[0].x)
    {
        m_WinData.m_Width = res[0].x;
        m_WinData.m_Height = res[0].y;
        return;
    }
    if (winX >= res[size-1].x)
    {
        m_WinData.m_Width = res[size-1].x;
        m_WinData.m_Height = res[size-1].y;
        return;
    }
    ...
    while (i < j) 
    {
        ...
        if (res[mid].x == winX)
        {
            m_WinData.m_Width = res[mid].x;
            m_WinData.m_Height = res[mid].y;
            return;
        }
        ...
        if (winX < res[mid].x)
        {
            if (mid > 0 && winX > res[mid - 1].x)
            {
                ...
                if (val == res[mid-1].x)
                {
                    m_WinData.m_Width = res[mid-1].x;
                    m_WinData.m_Height = res[mid-1].y;
                    return;
                }
                else
                {
